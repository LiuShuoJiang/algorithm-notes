...
- 用`top`表示栈顶所在的索引，初始时，`top = 0`，表示没有元素；
- `push x`：栈顶所在索引往后移动一格，然后放入`x`。`st[++top] = x`；
- `pop` : `top`往前移动一格。`top--`；
- `empty`：`top`等于`0`则栈非空，小于等于`0`则栈空。`top == 0 ? "YES" : "NO"`；
- `query`：返回栈顶元素，`st[top]`。
...
***单调栈*** 是栈的一种特殊形式，在栈中的元素必须满足单调性(一定是单调上升或单调下降等等的规律)。
...
[![单调栈](../../images/stack.gif)](https://www.acwing.com/solution/content/27437/)
...
> 例题：[滑动窗口](./sliding_windows.cpp)；[滑动窗口(STL解法)](./sliding_windows_2.cpp)
...
[![单调队列](../../images/monotonic_stack.png)](https://www.bilibili.com/video/BV1H5411j7o6)
