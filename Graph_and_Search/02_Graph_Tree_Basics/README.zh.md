...
补充：带权图也可以利用`struct`来实现链式前向星(一个表头数组悬挂多个链表)，使用**边集数组**`e[i]`存储第`i`条出边的终点`v`、边权`w`和下一条边`ne`；使用**表头数组**`h[u]`存储`u`点的***第一条***出边的编号；**边的编号**`idx`可取0,1,2,3...等，注意越往后插的边越靠近`h`，这样的插入方法称为**头插法**(在`DFS`中，后插的先被访问)
